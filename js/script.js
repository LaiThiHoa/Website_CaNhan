...
// 
document.querySelectorAll('.tab-lab').forEach(tab => {
    tab.addEventListener('click', () => {
        // Remove active class from all tabs and lab contents
        document.querySelectorAll('.tab-lab').forEach(t => t.classList.remove('active'));
        document.querySelectorAll('.lab-item').forEach(item => item.classList.remove('active'));

        // Add active class to the clicked tab and corresponding lab content
        tab.classList.add('active');
        document.getElementById(tab.getAttribute('data-lab')).classList.add('active');
    });
});
